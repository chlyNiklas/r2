...
class Orienter(Cordinator):
    feature_params = dict(maxCorners=100, qualityLevel=0.2, minDistance=7, blockSize=7) # Parameters for ShiTomasi corner detection
    lk_params =  dict(winSize=(15, 15), maxLevel=2,
                         criteria=(cv.TERM_CRITERIA_EPS | cv.TERM_CRITERIA_COUNT, 10, 0.03))# Parameters for Lucas-Kanade optical flow
    ...
    def __init__(self, feature_params=dict(), lk_params=dict(), max_distance: float=600.0) -> None:
        super().__init__()
    ...
    def process(self, old_frame: MatLike, new_frame: MatLike) -> None:
        ...
        if self.p0 is None or len(self.p0) < 10:
            new_points = cv.goodFeaturesToTrack(old_frame, mask=None, **feature_params) #type: ignore[type-var]
        ...
        p1, st, _ = cv.calcOpticalFlowPyrLK(old_gray, frame_gray, self.p0, None, **lk_params) # type: ignore[type-var]
        ...
        if p1 is not None:
            ...
            for new_frame, old_frame in zip(good_new, good_old):
                a, b = new_frame.ravel()
                dist = np.sqrt((a - self.frame_center[0])**2 + (b - self.frame_center[1])**2)
